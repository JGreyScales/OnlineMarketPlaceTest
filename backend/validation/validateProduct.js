    ...
    try {
        await validateGetProduct(req, res, (err) => {
            ...
            if (req.body.productName) {
                if (req.body.productName.length < Product.MIN_PRODUCT_NAME_LENGTH || req.body.productName.length > Product.MAX_PRODUCT_NAME_LENGTH) {
                    return res.status(400).json({error: `Invalid productName`})
                }
            }

            if (req.body.productBio){
                if (req.body.productBio.length < Product.MIN_PRODUCT_BIO_LENGTH || req.body.productBio.length > Product.MAX_PRODUCT_BIO_LENGTH) {
            ...
            if (req.body.productPrice){
                if (!isFinite(req.body.productPrice) || req.body.productPrice < Product.MIN_PRODUCT_PRICE) {
