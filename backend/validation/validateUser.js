...
const ALLOWED_FIELDS = ['userPhoto', 'userBio', 'userName', 'email', 'password', 'fundsAmount', 'rating', 'interests'];
...
async function validateGetUser(req, res, next) {
    ...
    if (req.params.amount){
        if (isNaN(req.params.amount) || req.params.amount < 1 || req.params.amount > 25){
